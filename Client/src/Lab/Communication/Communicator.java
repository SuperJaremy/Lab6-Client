...
public class Communicator {
    ...
    public void communicatorSend(String message) throws IOException,SocketException{
        if(isOpened) {
            ms = new MessageSender(ds);
            ms.sendMessage(new MessageFormer(message));
        }
        else
    ...
    public void communicatorSend(Vector<String> list)throws IOException{
        if(isOpened) {
            ms = new MessageSender(ds);
            ms.sendMessage(new MessageFormer(list));
        }
        else throw new SocketException();
    }
